          ...
          <>
            <div className="text-center mb-8">
              ...
              <p className="text-lg text-cyan-700">
                Select from our comfortable accommodations
              </p>
            </div>
            ...
            {isLoading ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {Array.from({ length: 8 }).map((_, i) => (
                  <Card key={i} className="border-cyan-200">
                    <div className="aspect-[3/2] overflow-hidden rounded-t-lg bg-cyan-100 animate-pulse" />
                    <CardContent className="p-3 space-y-2">
                      <div className="flex justify-between items-start">
                        <div className="flex-1 space-y-1">
                          <div className="h-4 bg-cyan-100 rounded animate-pulse w-3/4" />
                          <div className="h-3 bg-cyan-50 rounded animate-pulse w-1/2" />
                        </div>
                        <div className="ml-2 space-y-1 text-right">
                          <div className="h-4 bg-cyan-100 rounded animate-pulse w-16" />
                          <div className="h-3 bg-cyan-50 rounded animate-pulse w-12" />
                        </div>
                      </div>
                      <div className="flex gap-1">
                        <div className="h-5 bg-cyan-100 rounded animate-pulse w-16" />
                        <div className="h-5 bg-cyan-100 rounded animate-pulse w-14" />
                      </div>
                      <div className="h-7 bg-cyan-100 rounded animate-pulse w-full" />
                    </CardContent>
                  </Card>
                ))}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                {rooms.map((room) => (
                  <Card
                    key={room.id}
                    className="border-cyan-200 hover:shadow-lg transition-shadow"
                  >
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between items-start">
                          <div className="flex-1 min-w-0">
                            <h3 className="font-semibold text-cyan-800 text-sm truncate">
                              {room.name}
                            </h3>
                        ...
                        <div className="flex flex-wrap gap-1">
                          {room.amenities.slice(0, 2).map((amenity, index) => (
                            <span
                              key={index}
                              className="px-1.5 py-0.5 bg-cyan-100 text-cyan-800 text-xs rounded"
                            >
                              {amenity}
                            </span>
                          ))}
                ...
                <Card className="border-cyan-200">
                  <CardHeader>
                    ...
                    <CardDescription className="text-lg">
                      {selectedRoom?.type} • Up to {selectedRoom?.capacity}{" "}
                      guests
                    </CardDescription>
                  </CardHeader>
              ...
              <div>
                <Card className="border-cyan-200 sticky top-4">
                  <CardHeader>
                    ...
                    <CardDescription>
                      Fill in your details to reserve this room
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <form onSubmit={handleBookingSubmit} className="space-y-6">
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          ...
                          <SelectContent>
                            {Array.from(
                              { length: selectedRoom?.capacity || 1 },
                              (_, i) => (
                                <SelectItem
                                  key={i + 1}
                                  value={(i + 1).toString()}
