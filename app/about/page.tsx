  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        <div
          className={`flex-shrink-0 h-11 w-11 rounded-full bg-gradient-to-br ${cfg.avatarGrad} text-white font-bold text-lg flex items-center justify-center shadow-md`}
        >
...
function TeamModal({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`bg-gradient-to-br ${cfg.avatarGrad} px-8 pt-10 pb-16 relative`}
        >
          <button
            onClick={onClose}
        ...
        <div className="-mt-8 rounded-t-3xl bg-white relative z-10 px-8 pt-7 pb-8">
          <div
            className={`inline-flex items-center gap-2 rounded-xl border px-3 py-1.5 mb-5 ${cfg.badge} text-xs font-semibold`}
          >
      ...
      try {
        ...
        if (result?.data) {
          ...
          setAboutData({
            ...
            storyImages: toArray<string>(d.storyImages).map(rewriteImageUrl),
          });
        }
      } catch (err) {
    ...
    <div className="min-h-screen relative bg-gradient-to-b from-slate-50 via-blue-50 to-teal-50">
      ...
      <section className="relative min-h-[200px] flex items-center justify-center py-20">
        ...
        <div className="relative z-10 container mx-auto px-4 text-center">
          ...
          <p className="text-md sm:text-lg md:text-xl text-slate-700 max-w-2xl mx-auto mb-8 leading-relaxed font-light bg-white/80 p-6 rounded-2xl backdrop-blur-sm border border-teal-200 shadow-sm">
            Your gateway to world-class diving experiences in the Philippines'
            diving capital since 2010
          </p>
      ...
      <section className="relative py-20 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-center text-xs font-semibold tracking-[0.2em] uppercase text-teal-700 mb-4">
            Our Journey
          </p>
            ...
            <div className="space-y-4 max-w-xl mx-auto w-full">
              {loading ? (
                Array.from({ length: 3 }).map((_, i) => (
                  <div
                    key={i}
                    className="flex gap-5 bg-white border border-slate-100 rounded-2xl p-6 shadow-md animate-pulse"
                  >
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
            {loading ? (
              Array.from({ length: 3 }).map((_, i) => (
                <div
                  key={i}
                  className="bg-white border border-slate-100 rounded-2xl p-7 shadow-md animate-pulse space-y-3"
                >
              ...
              <p className="text-slate-400 text-sm text-center col-span-3 py-10">
                No core values yet.
              </p>
            ) : (
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
              {Array.from({ length: 6 }).map((_, i) => (
                <div
                  key={i}
                  className="rounded-2xl border border-slate-100 p-5 animate-pulse flex items-center gap-3"
                >
                  ...
                  return (
                    <div key={lvl} className="relative">
                      <div className="flex items-center justify-center mb-10">
                        <div
                          className={`flex items-center gap-2 px-5 py-2 rounded-full border text-xs font-bold uppercase tracking-wider shadow-sm ${cfg.badge}`}
                        >
      ...
      <section className="py-20 bg-gradient-to-br from-slate-900 via-blue-950 to-teal-950 relative overflow-hidden">
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse 80% 60% at 50% 100%, rgba(20,184,166,0.6) 0%, transparent 70%)",
          }}
        />
        <div className="container mx-auto px-6 text-center relative z-10">
          ...
          <p className="text-slate-400 max-w-xl mx-auto mb-10 text-base leading-relaxed">
            Join thousands of divers who've discovered the magic of Anilao's
            underwater world. Book your dive or certification course today.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/booking">
              <Button
                size="lg"
                className="min-w-[220px] bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-600 hover:to-cyan-600 text-white px-8 py-5 text-base font-bold rounded-2xl shadow-xl border-0"
              >
            ...
            <Link href="/certification">
              <Button
                size="lg"
                variant="outline"
                className="min-w-[220px] border-2 border-teal-500/50 text-teal-300 hover:bg-teal-900/40 bg-transparent px-8 py-5 text-base font-bold rounded-2xl"
              >
