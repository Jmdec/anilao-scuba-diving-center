...
const AdminTestimonialsPage = () => {
  ...
  const handleDeleteTestimonial = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `/api/testimonials/${testimonialToDelete.id}`,
        {
          method: "DELETE",
          headers: {
            ...(token && { Authorization: `Bearer ${token}` }),
  ...
  const handleQuickStatusUpdate = async (
    ...
    // Optimistically update UI immediately so it never disappears
    setTestimonials((prev) =>
      prev.map((t) =>
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        `/api/testimonials/${testimonial.id}/status`,
        {
          method: "PATCH",
          headers: {
            ...(token && { Authorization: `Bearer ${token}` }),
      ...
      if (response.ok && data.success) {
        ...
        setTestimonials((prev) =>
          prev.map((t) =>
            t.id === testimonial.id ? { ...t, status: testimonial.status } : t,
          ),
        );
        toast({
          title: "Error",
  ...
  const filteredTestimonials = testimonials.filter((testimonial) => {
    ...
    const matchesExperience =
      ...
      testimonial.diving_experience === experienceFilter;
    const matchesStatus =
  ...
  const totalTestimonials = testimonials.length; // total from ALL, not filtered
                        ...
                        {testimonial.diving_experience ? (
                          <Badge variant="outline">
                            {experienceOptions.find(
                              (e) => e.value === testimonial.diving_experience,
                            )?.label || testimonial.diving_experience}
                          </Badge>
                        ) : (
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          {testimonial.status !== "approved" && (
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                handleQuickStatusUpdate(testimonial, "approved")
                              }
                          ...
                          {testimonial.status !== "rejected" && (
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                handleQuickStatusUpdate(testimonial, "rejected")
                              }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setFormData({ ...formData, diving_experience: value })
                  }
                >
                  <SelectTrigger>
        ...
        <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="edit-diving_experience">
                  Diving Experience
                </Label>
                <Select
                  value={formData.diving_experience}
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                ...
                <Badge className={getStatusColor(selectedTestimonial.status)}>
                  {statusOptions.find(
                    (s) => s.value === selectedTestimonial.status,
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  {selectedTestimonial.diving_experience
                    ? experienceOptions.find(
                        (e) =>
                          e.value === selectedTestimonial.diving_experience,
                      )?.label
                    : "Not specified"}
                </div>
                <div>
                  <strong>Course Taken:</strong>{" "}
                  {selectedTestimonial.course_taken || "Not specified"}
                </div>
              </div>
              <div>
