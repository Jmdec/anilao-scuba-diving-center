...
const STORY_IMAGE_OPTIONS = [
  ...
  {
    label: "Underwater tropical scene",
    value: "/uploads/1755248496669_underwater-tropical-scene.png",
  },
  {
    label: "Diving coral reef",
    value: "/underwater-coral-reef-with-colorful-fish-and-marin.png",
  },
  {
    label: "Anilao reef adventure",
    value: "/underwater-coral-reef-anilao-diving.png",
  },
  {
    label: "Diver exploring coral reef",
    value: "/scuba-diver-exploring-coral-reef.png",
  },
];
...
export default function AboutAdminPage() {
    ...
    async function loadData() {
      try {
        ...
        if (result?.data) {
          ...
          const storyImages = toArray<string>(d.storyImages).map(
            rewriteImageUrl,
          );
          const teamMembers = toArray<TeamMember>(d.teamMembers).map((m) => ({
            ...m,
            level: (m.level ?? "staff") as TeamLevel,
          }));
  ...
  const removeStoryItem = (index: number) =>
    confirmToast("Remove this story block?", () => {
      setAboutData((cur) => ({
        ...cur,
        stories: cur.stories.filter((_, i) => i !== index),
      }));
  ...
  const addValueCard = () =>
    confirmToast("Add a new core value?", () => {
      setAboutData((cur) => ({
        ...
        values: [
          ...cur.values,
          { icon: "Star", title: "New Value", body: "Describe this value." },
        ],
      }));
  ...
  const removeValueItem = (index: number) =>
    confirmToast("Remove this core value?", () => {
      setAboutData((cur) => ({
        ...cur,
        values: cur.values.filter((_, i) => i !== index),
      }));
  ...
  // ── Styles ────────────────────────────────────────────────────────────────
  const inputClass =
    "w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-cyan-400 focus:outline-none";
  const textareaClass =
    "w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-900 shadow-sm focus:border-cyan-400 focus:outline-none min-h-[100px]";
  const labelClass = "mb-1 block text-sm font-medium text-slate-700";
  const cardClass =
    "relative rounded-2xl border border-slate-200 bg-slate-50 p-5 mb-4 space-y-4";

  const teamByLevel = LEVEL_ORDER.reduce<
    Record<TeamLevel, { member: TeamMember; globalIndex: number }[]>
  >(
    (acc, lvl) => {
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">
              Manage story, images, values, and team members.
            </p>
          </div>
        ...
        <div className="flex border-b border-slate-200 overflow-x-auto">
          {TABS.map((tab) => (
            <button
              ...
              className={`flex-shrink-0 px-6 py-4 text-sm font-semibold transition-colors whitespace-nowrap ${
                activeTab === tab.id
        ...
        <div className="p-6">
          ...
          {activeTab === "story" && (
            <div>
              <div className="flex items-center justify-between mb-5">
                <p className="text-sm text-slate-500">
                  {aboutData.stories.length} block
                  {aboutData.stories.length !== 1 ? "s" : ""}
                </p>
                <Button variant="secondary" onClick={addStoryBlock}>
                  Add Block
                </Button>
              </div>
              {loading ? (
                ...
                aboutData.stories.map((item, index) => (
                  <div
                    key={index}
                    ref={
                      index === aboutData.stories.length - 1
                        ? lastAddedRef
                        : null
                    }
                    className={cardClass}
                  >
                    ...
                    <label className="block">
                      ...
                      <input
                        ...
                        value={item.heading}
                        onChange={(e) =>
                          updateStoryItem(index, "heading", e.target.value)
                        }
                        onBlur={() =>
                          notify("Story updated", "Heading updated.")
                        }
                      />
                    </label>
                    <label className="block">
                      ...
                      <textarea
                        ...
                        value={item.content}
                        onChange={(e) =>
                          updateStoryItem(index, "content", e.target.value)
                        }
                        onBlur={() =>
                          notify("Story updated", "Content updated.")
                        }
                      />
                    </label>
                  </div>
          ...
          {activeTab === "images" && (
            <div>
              <div className="flex items-center justify-between mb-5">
                <p className="text-sm text-slate-500">
                  {aboutData.storyImages.length} image
                  {aboutData.storyImages.length !== 1 ? "s" : ""}
                </p>
                <Button variant="secondary" onClick={addStoryImage}>
                  Add Image
                </Button>
              </div>
              {aboutData.storyImages.length === 0 ? (
                ...
                aboutData.storyImages.map((src, index) => {
                  ...
                  const previewSrc =
                    (uploadingIndex === index ? localPreviews[index] : null) ??
                    src ??
                    localPreviews[index];
                  return (
                    <div
                      key={index}
                      ref={
                        index === aboutData.storyImages.length - 1
                          ? lastAddedRef
                          : null
                      }
                      className={cardClass}
          ...
          {activeTab === "values" && (
            <div>
              <div className="flex items-center justify-between mb-5">
                <p className="text-sm text-slate-500">
                  {aboutData.values.length} value
                  {aboutData.values.length !== 1 ? "s" : ""}
                </p>
                <Button variant="secondary" onClick={addValueCard}>
                  Add Value
                </Button>
              </div>
              {aboutData.values.length === 0 ? (
                ...
                aboutData.values.map((item, index) => (
                  <div
                    key={index}
                    ref={
                      index === aboutData.values.length - 1
                        ? lastAddedRef
                        : null
                    }
                    className={cardClass}
                  >
                    ...
                    <div className="flex items-center gap-3">
                      <div>
                        ...
                        <IconPreview
                          name={item.icon || iconFromTitle(item.title)}
                        />
                      </div>
                      <label className="block flex-1">
                        ...
                        <input
                          ...
                          onBlur={() =>
                            notify("Value updated", "Title and icon updated.")
                          }
                        />
                      </label>
                    </div>
                    <label className="block">
                      ...
                      <textarea
                        ...
                        value={item.body}
                        onChange={(e) =>
                          updateValueItem(index, "body", e.target.value)
                        }
                        onBlur={() =>
                          notify("Value updated", "Description updated.")
                        }
                      />
                    </label>
                  </div>
          ...
          {activeTab === "team" && (
            <div>
              <div className="flex items-center justify-between mb-5">
                <p className="text-sm text-slate-500">
                  {aboutData.teamMembers.length} member
                  {aboutData.teamMembers.length !== 1 ? "s" : ""}
                </p>
                  ...
                  return (
                    <div key={lvl} className="mb-8">
                      <div className="flex items-center gap-3 mb-3">
                        <span
                          className={`text-xs font-bold uppercase tracking-widest px-3 py-1 rounded-full border ${meta.badge} ${meta.color}`}
                        >
                          {meta.label}
                        </span>
                        <span className="text-xs text-slate-400">
                      ...
                      {group.map(({ member, globalIndex }) => (
                        <div
                          key={globalIndex}
                          ref={
                            globalIndex === aboutData.teamMembers.length - 1
                              ? lastAddedRef
                              : null
                          }
                        ...
                        >
                          ...
                          <div className="grid gap-4 sm:grid-cols-3">
                            <label className="block">
                              ...
                              <input
                                ...
                                onBlur={() =>
                                  notify("Team updated", "Name updated.")
                                }
                              />
                            </label>
                            <label className="block">
                              ...
                              <input
                                ...
                                onBlur={() =>
                                  notify("Team updated", "Role updated.")
                                }
                              />
                            </label>
                            <label className="block">
                          ...
                          <label className="block">
                            ...
                            <textarea
                              ...
                              onBlur={() =>
                                notify("Team updated", "Bio updated.")
                              }
                            />
                          </label>
                        </div>
