...
export async function PUT(
  ...
  try {
    ...
    if (!statusResponse.ok) {
      ...
      throw new Error(
        `Failed to update status: ${statusResponse.status} ${statusResponse.statusText}`,
      );
    }
    ...
    if (status === "completed") {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to fetch applications: ${response.status} ${statusResponse.statusText}`,
        );
      }
      ...
      const applicationData = applications.find(
        (app: any) => app.id === Number.parseInt(applicationId),
      );
      ...
      const application = {
        ...
        certification: {
          name:
            applicationData.certification?.name ||
            applicationData.certification_name ||
            "Unknown Certification",
        },
        completed_at:
          applicationData.completed_at ||
          applicationData.completion_date ||
