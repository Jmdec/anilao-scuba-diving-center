...
async function writeAboutData(data: unknown) {
    try {
        ...
        await fs.writeFile(aboutFilePath, JSON.stringify(data, null, 2), "utf-8");
        return true;
    } catch (error) {
...
export async function GET() {
    ...
    if (!data) {
        return NextResponse.json({ success: false, message: "Unable to load about content" }, { status: 500 });
    }
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!payload || typeof payload !== "object") {
            return NextResponse.json({ success: false, message: "Invalid about payload" }, { status: 400 });
        }

        const saved = await writeAboutData(payload);
        if (!saved) {
            return NextResponse.json({ success: false, message: "Could not save about content" }, { status: 500 });
        }

        return NextResponse.json({ success: true, message: "About content updated", data: payload });
    } catch (error) {
