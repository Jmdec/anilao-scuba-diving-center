...
export async function GET(
  ...
  try {
    if (!LARAVEL_API_URL) {
      console.error(
        "[v0] NEXT_PUBLIC_API_URL or LARAVEL_API_URL not configured",
      );
    ...
    console.log(
      `[v0] Fetching room ${roomId} from Laravel backend: ${LARAVEL_API_URL}/rooms/${roomId}`,
    );
    ...
    if (!response.ok) {
      console.error(
        `[v0] Laravel API error: ${response.status} ${response.statusText}`,
      );
      return NextResponse.json(
        {
          success: false,
...
export async function PUT(
  ...
  try {
    ...
    console.log(
      `[v0] Updating room ${roomId} via Laravel backend using POST with method override`,
    );
